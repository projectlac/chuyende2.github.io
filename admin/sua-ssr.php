  ...
  foreach($_FILES['photos']['name'] as $name => $value){
    ...
    if(in_array($ext,$valid_formats)||in_array($ext1,$valid_formats)){
      ...
      if ($size < (MAX_SIZE*1024)){
        ...
        if(move_uploaded_file($_FILES['photos']['tmp_name'][$name], $newname)){
           ...
           mysqli_query($conn,"UPDATE optionsq2 SET hinh='$image_name',SSR_name='$SSR_name',cottruyen='$cottruyen','optionsQ2_value'='$image_name1' WHERE optionsQ2_id='$id'");
           echo "<script>
    var r = confirm('sửa SSR thành công');
      if (r == true) {
    ...
    var r = confirm('Có lỗi');
      if (r == true) {
       ...
       echo "<script>
    var r = confirm('Dung lượng ảnh quá lớn');
      if (r == true) {
        window.location.replace('thongtin-ssr.php');
        } else {window.location.replace('thongtin-ssr.php');}
    </script> ";
     ...
     echo "<script>
    var r = confirm('Vui long chọn ảnh');
      if (r == true) {
        window.location.replace('thongtin-ssr.php');
        } else {window.location.replace('thongtin-ssr.php');}
    </script> ";
    ...
    $("#imageform").ajaxForm({
      ...
      <?php  }?>
           ...
           <div class="noidung-moi">
              <form action="sua-ssr.php?do=sua-ssr&id=<?=$chitietSSR['optionsQ2_id']?>" method="POST" enctype="multipart/form-data" >
              <div class="wrap-input100 validate-input" data-validate = "User is required">
                  <input class="input100" type="text" name="name" value="<?=$chitietSSR['SSR_name']?>" style="height: 30px;">
                  <span class="focus-input100"></span>
                  <span class="label-input100"></span>
              </div>
              <div class="col-md-6">
                <textarea name="content">
               <?=$chitietSSR['cottruyen']?>
              </textarea>
              </div>
              <div class="col-md-6">
               <div class="wrap-input100 validate-input" data-validate = "User is required">
                <!--  <div id='imageloadstatus' style='display:none'><img src="images/loader.gif" alt="Uploading...."/></div> -->
                 <div id='imageloadbutton'>
                     ...
                     <input type="file" name="photos[]" id="photoimg" multiple="true" value="<?=$chitietSSR['hinh']?>"/>
                     <input type="file" name="photos1[]" id="photoimg1" multiple="true" value="<?=$chitietSSR['optionsQ2_value']?>"/>
                  </div>
                  <span class="focus-input100"></span>
